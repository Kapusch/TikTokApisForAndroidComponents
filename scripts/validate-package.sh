...
if grep -Eq "tiktok-open-sdk-auth|TikTokAuth" <<<"$entries"; then
...
grep -Fq 'TikTokShareActivity' "$tmp_dir/wrapper-manifest.xml"
grep -Fq '${tiktokClientKey}' "$tmp_dir/wrapper-manifest.xml"
unzip -p "$tmp_dir/android/aar/tiktok-open-sdk-share-2.3.1.aar" classes.jar > "$tmp_dir/share-classes.jar"
jar tf "$tmp_dir/share-classes.jar" | grep -Eq '^com/tiktok/open/sdk/share/ShareApi.class$'

grep -Eq 'v2\.3\.1' "$repo_root/DependencyLocks/Android/lockstate.txt"
verification="$repo_root/src/Kapusch.TikTokApisForAndroidComponents/Native/Android/gradle/verification-metadata.xml"
grep -Eq '<sha256 value="[0-9a-f]{64}"' "$verification"
